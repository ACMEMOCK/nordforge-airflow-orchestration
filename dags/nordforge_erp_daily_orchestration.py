...
EXPORT_ROOT = Path(os.getenv("NFORGE_EXPORT_ROOT", "/opt/airflow/data/exports"))
CONTROL_DIR = Path("/opt/airflow/data/control")
DEMO_MODE = os.getenv("NFORGE_DEMO_MODE", "true").strip().lower() in {"1", "true", "yes", "y"}
...
def _csv_row_count(zf: zipfile.ZipFile, name: str) -> int:
...
@dag(
    ...
    description="Daily orchestration for NordForge ERP extracts, inventory, logistics, freight, and customer-service data products.",
    schedule="15 5 * * *",
    start_date=datetime(2026, 6, 24),
    catchup=False,
    default_args=DEFAULT_ARGS,
    tags=["nordforge", "erp", "industrial", "essential", "data-platform"],
)
def nordforge_erp_daily_orchestration():
    ...
    @task
    def validate_export_packages(contracts: dict[str, Any]) -> dict[str, Any]:
        package_results = []
        issues = []

        for package in contracts["packages"]:
            package_dir = EXPORT_ROOT / package["local_export_folder"]
            zip_path = package_dir / package["expected_zip_file"]
            expected_csvs = {
            ...
            if not zip_path.exists():
                result = {
                    ...
                    "status": "not_mounted_demo" if DEMO_MODE else "missing",
                    "zip_path": str(zip_path),
                    "expected_csv_files": len(expected_csvs),
                    "validated_csv_files": 0,
                    "expected_detail_rows": package["detail_rows_preserved"],
                    "validated_detail_rows": None,
                    "issues": [] if DEMO_MODE else [f"Missing zip package: {zip_path}"],
                }
            ...
            with zipfile.ZipFile(zip_path) as zf:
                ...
                missing = sorted(set(expected_csvs) - actual_names)
                unexpected = sorted(actual_names - set(expected_csvs))
                row_mismatches = []

                for csv_name, expected_rows in expected_csvs.items():
                    if csv_name not in actual_names:
                        continue
                    actual_rows = _csv_row_count(zf, csv_name)
            ...
            result_issues = []
            if missing:
                result_issues.append(f"Missing CSV files: {missing}")
            if unexpected:
                result_issues.append(f"Unexpected CSV files: {unexpected}")
            if row_mismatches:
                result_issues.append(f"Row count mismatches: {row_mismatches}")

            primary_table = next((t for t in package["tables"] if t.get("include_in_csv", True) and t.get("csv_file")), {})
            validated_detail_rows = None
            if primary_table.get("csv_file") and primary_table["csv_file"] in expected_csvs:
                validated_detail_rows = expected_csvs[primary_table["csv_file"]]
            ...
            package_results.append(
                {
                    ...
                    "domain": package["domain"],
                    "pipeline_group": package["pipeline_group"],
                    "status": "valid" if not result_issues else "invalid",
                    "zip_path": str(zip_path),
                    "expected_csv_files": len(expected_csvs),
                    "validated_csv_files": len(actual_names),
                    "expected_detail_rows": package["detail_rows_preserved"],
                    "validated_detail_rows": validated_detail_rows,
        ...
        if issues:
            raise ValueError(json.dumps({"validation_issues": issues}, indent=2))
    ...
    def publish_control_manifest(
        ...
        manifest_path = CONTROL_DIR / f"nordforge_orchestration_manifest_{run_id}.json"
        manifest = {
            "run_id": run_id,
            "company": contracts["company"],
            "contract_version": contracts["contract_version"],
        ...
        return {"manifest_path": str(manifest_path), "run_id": run_id}

    @task
    def publish_retry_queue(validation: dict[str, Any]) -> dict[str, Any]:
        retry_candidates = []
                ...
                retry_candidates.append(
                    {
                        ...
                        "action": "monitor_demo_mount" if result["status"] == "not_mounted_demo" else "retry_source_interface",
                        "owner_hint": "NordForge Data Platform",
                    }
                )
        return {
    ...
    @task
    def publish_kpi_manifest(contracts: dict[str, Any], validation: dict[str, Any]) -> dict[str, Any]:
        totals_by_group: dict[str, int] = {}
        rows_by_domain: dict[str, int] = {}
        for package in contracts["packages"]:
            totals_by_group[package["pipeline_group"]] = totals_by_group.get(package["pipeline_group"], 0) + package["detail_rows_preserved"]
            rows_by_domain[package["domain"]] = package["detail_rows_preserved"]

        valid_packages = sum(1 for result in validation["package_results"] if result["status"] == "valid")
        return {
            "published_at_utc": _utc_now_iso(),
            "package_count": len(contracts["packages"]),
            "valid_package_count": valid_packages,
            "contracted_detail_rows": sum(rows_by_domain.values()),
